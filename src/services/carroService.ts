...
export class CarroService {
    ...
    async insereCarro(carroBody: any): Promise<Carro> {
        ...
        if(await this.carroRepository.placaRepetida(carroBody.placa) === undefined){
    ...
    async listaCarros(): Promise<Carro[]> {
        if(await this.carroRepository.listaCarros() === undefined){
    ...
    async listaCarroID(id: any): Promise<Carro | undefined>{
        if(await this.carroRepository.listaCarroID(Number(id)) === undefined){
            this.erroStatus.insereErro(404)
    ...
    async listaCarroDisponivel(): Promise<any[] | undefined >{
        if(await this.estoqueRepository.carrosDisponiveis() === undefined){
            this.erroStatus.insereErro(404)
            throw new Error("Nenhum veiculo cadastrado")
        }
        return this.estoqueRepository.carrosDisponiveis()
    }

    async atualizaCarro(id: any, carroBody: any): Promise<Carro | undefined>{
        if(await this.carroRepository.listaCarroID(Number(id)) === undefined){
            this.erroStatus.insereErro(404)
            throw new Error("Carro com este ID, não existe no sistema.")
        }
    ...
    async deleteCarro(id: any): Promise<Carro | undefined>{
        ...
        if(this.estoqueRepository.listaEstoqueCarroID(Number(id)) != undefined){
            this.erroStatus.insereErro(422)
